...
import numpy as np
from pydestruct.nn.dropout import IndependentDropout
...
class BatchIndices:
    ...
    def __init__(self, batch_idxs_np, device):
        ...
        self.batch_idxs_torch = torch.from_numpy(batch_idxs_np).to(device)
...
class FeatureDropout(nn.Module):
    ...
    def forward(self, input, batch_idxs):
        noise = input.new().resize_(batch_idxs.batch_size, input.size(1))
        ...
        noise = noise[batch_idxs.batch_idxs_torch, :]
        output = input.mul(noise)
...
class Encoder(nn.Module):
    ...
    def forward(self, res, timing_signal, batch_idxs):
